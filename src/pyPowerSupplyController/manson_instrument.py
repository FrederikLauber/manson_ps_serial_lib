...
from .mi_commands import MICMD_GETM, MICMD_GETS, MICMD_GETD, MICMD_SOVP, MICMD_SVSH, MICMD_SISH, MICMD_SOCP, \
    MICMD_CURR, MICMD_GMOD, MICMD_GVER, MICMD_GABC, MICMD_SCHA, MICMD_GCHA, MICMD_GMIN, MICMD_SETD, MICMD_GMAX, \
    MICMD_PROM, MICMD_RUNM, MICMD_SABC, MICMD_ENDS, MICMD_SESS, MICMD_GOCP, MICMD_GISH, MICMD_GOVP, MICMD_GVSH, \
    MICMD_VOLT, MICMD_SOUT, MICMD_GOUT
from .emulated_instrument_serial import EmulatedInstrumentSerial
from .exceptions import (CouldNotConnectError, FunctionNotSupportedForModelError, InvalidModelError,
                         NotConnectedError, UnsupportedModelError)
...
from .serializer import SZR_VTYPE_CURR, SZR_VTYPE_VOLT, SZR_VTYPE_IX, SZR_VTYPE_VARVOLT, SZR_VTYPE_VARCURR, \
    SZR_VTYPE_SPECVOLT, SZR_VTYPE_SPECCURR, SZR_VTYPE_MODE, SZR_VTYPE_RANGE, SZR_VTYPE_STATE, SZR_OUTP_MODE_CC, \
    SZR_OUTP_MODE_CV, SZR_VTYPE_VER, SZR_VTYPE_MODEL, Serializer
...
class MansonInstrument(object):
    ...
    def open_port(self, comPort, emulateModel=None):
        ...
        try:
            if comPort != VIRTUAL_SERIAL_DEVICE:
                self._pyserObj = pyser_Serial(comPort, baudrate=self._BAUDRATE, bytesize=8, parity="N", stopbits=1,
                                              timeout=0.1)
    ...
    def set_output_state(self, state):
        ...
        """
        assert state is True or state is False, "state needs to be bool"
        #
        cmd = MICMD_SOUT
    ...
    def load_memory_preset(self, index):
        ...
        assert 0 <= index < hwSpecs["realMemPresetLocations"], "index out of range (0..%d)" % (
                hwSpecs["realMemPresetLocations"] - 1)
        #
    ...
    def _lowlev_send_cmd(self, cmd, cargs, extraWait=False):
        ...
        assert extraWait is True or extraWait is False, "extraWait needs to be bool"
        #
        self._check_hwCmdSupp(cmd)
        #
        if self._pyserObj is None:
    ...
    def _get_cmd_set_volt_or_curr(self, valFloat, varName, isVolt):
        ...
        assert isVolt is True or isVolt is False, "isVolt needs to be bool"
        #
