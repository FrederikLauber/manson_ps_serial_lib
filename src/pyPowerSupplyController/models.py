...
from .mi_commands import MICMD_GMOD, MICMD_GVER, MICMD_GOUT, MICMD_SOUT, MICMD_ENDS, MICMD_SESS, MICMD_VOLT, \
    MICMD_GETD, MICMD_GETS, MICMD_GMIN, MICMD_GMAX, MICMD_GETM, MICMD_PROM, MICMD_RUNM, MICMD_SABC, MICMD_GABC, \
    MICMD_SOVP, MICMD_SOCP, MICMD_GOVP, MICMD_GOCP, MICMD_SVSH, MICMD_SISH, MICMD_GVSH, MICMD_GISH, MICMD_SETD, \
    MICMD_CURR, MICMD_GCHA, MICMD_SCHA
...
def get_hw_model_id(modelId):
    ...
    if match:
        resS = match.group()
        if resS not in MODEL_LIST_SERIES_ALL:
...
def build_spec_dict(mnv=0.0, mxv=0.0, mnc=0.0, mxc=0.0, pv=0, pc=0, virtMpl=0, realMpl=0, td=0, charStateOn="",
                    charStateOff="", rg=0, modelSeries="", modelSubSeries=""):
    ...
    return {
        ...
        "totalDigits": td,
        # total amount of digits that Volt/Current values can have, e.g. 4 ^= values will have the format x.yyy or xx.yy or xxx.y
